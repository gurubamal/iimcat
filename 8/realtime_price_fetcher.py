...
======================
Fetches current prices from NSE Direct API (preferred) and yfinance (fallback)
to ensure AI doesn't rely on training data.

This module ensures that:
1. AI receives ONLY real-time data from NSE Direct API or yfinance
...
✅ UPGRADED: Now uses NSE Direct API for more current prices (~0 delay vs ~15 min delay)
...
Usage:
    ...
    #   'symbol': 'RELIANCE.NS',
    #   'source': 'NSE_DIRECT',  # or 'yfinance' if fallback
...
# Import NSE data fetcher (new primary source)
try:
    from nse_data_fetcher import get_realtime_price as get_nse_price
    NSE_FETCHER_AVAILABLE = True
except ImportError:
    NSE_FETCHER_AVAILABLE = False
    print("⚠️  WARNING: NSE data fetcher not available. Will use yfinance only.", file=sys.stderr)
...
def fetch_current_price(ticker: str,
                        ...
                        fallback_suffix: str = '.BO',
                        prefer_nse: bool = True) -> Optional[Dict]:
    """
    Fetch current price from NSE Direct API (preferred) or yfinance (fallback).

    Priority:
    1. NSE Direct API (real-time, ~0 delay) - PREFERRED during market hours
    2. yfinance (15-min delay) - Fallback if NSE fails or outside market hours

    Args:
        ticker: Stock symbol (e.g., 'RELIANCE')
        exchange_suffix: NSE suffix (default: '.NS')
        fallback_suffix: BSE suffix (default: '.BO')
        prefer_nse: Try NSE Direct API first (default: True)

    Returns:
        dict with keys: current_price, timestamp, symbol, source, data_available
        or None if fetch fails
    """
    # Normalize ticker (remove exchange suffixes for NSE API)
    ...
    if prefer_nse and NSE_FETCHER_AVAILABLE:
        try:
            ...
            if nse_data and nse_data.get('price') is not None:
                # NSE data found - format to match our standard structure
                price_snapshot = {
                    'current_price': float(nse_data['price']),
                    'timestamp': nse_data['timestamp'].isoformat() if hasattr(nse_data['timestamp'], 'isoformat') else str(nse_data['timestamp']),
                    'symbol': f"{clean_ticker}.NS",
                    'data_available': True,
                    'source': nse_data.get('source', 'NSE_DIRECT'),
                    'market_status': nse_data.get('market_status', 'UNKNOWN'),
            ...
            print(f"⚠️  NSE Direct API failed for {clean_ticker}: {str(e)[:100]}", file=sys.stderr)
            # Continue to yfinance fallback

    # Strategy 2: Fallback to yfinance
    if not YFINANCE_AVAILABLE:
        # If both NSE and yfinance unavailable, try cache
        ...
        return {
            ...
            'error': 'No data sources available (NSE failed, yfinance not installed)'
        }

    # Try NSE first, then BSE via yfinance
    symbols_to_try = []
    if '.' in ticker:
    ...
    return {
        ...
        'symbol': ticker,
        'data_available': False,
        'error': f'No data for {ticker} (tried NSE Direct + yfinance: {", ".join(symbols_to_try)})'
    }
...
"""
    ...
    source_note = ""
    if source == 'NSE_DIRECT':
        source_note = "\n✅ Using NSE Direct API (real-time, ~0 delay - most current available)"
    elif source == 'yfinance.fast_info':
        source_note = "\n⚠️  Using yfinance fast_info (~15 min delay - NSE was unavailable)"
    elif source == 'yfinance.history':
        source_note = "\n⚠️  Using yfinance history (last close - NSE was unavailable)"

    market_status = price_data.get('market_status', '')
    market_note = f"\nMarket Status: {market_status}" if market_status else ""

    return f"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎯 REAL-TIME PRICE DATA (FETCHED NOW FROM {source.upper()})
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Ticker: {ticker}
Current Price: ₹{cp:.2f}
Fetched At: {ts}
Source: {source}{source_note}{market_note}
...
1. Use ONLY the above current price (₹{cp:.2f}) fetched just now from {source}
